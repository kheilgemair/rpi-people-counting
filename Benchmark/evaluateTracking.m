function [allMets, metsBenchmark, metsMultiCam] = evaluateTracking(seqmap, resDir, gtDataDir,benchmarkGt_name, benchmark)
...
for ind = 1:length(allSequences)
    ...
    if ~multicam
        ...
        if benchmarkGt_name ~= ""
            gtFilename = fullfile(gtDataDir,convertStringsToChars(benchmarkGt_name),'gt','gt.txt');
            sequenceFolder = [gtDataDir, convertStringsToChars(benchmarkGt_name), filesep];

        else 
            gtFilename = fullfile(gtDataDir,sequenceName,'counter_gt','gt.txt');
